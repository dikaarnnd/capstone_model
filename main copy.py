...
class UserInput(BaseModel):
    ...
    Name: str
    Height: float  # Tinggi dalam meter (contoh: 1.75)
    Weight: float  # Berat dalam kg (contoh: 75.0)
    Target_Weight_Loss: float # Target turun dalam kg (contoh: 4.0)

    # --- Variabel Asli untuk Deep Learning ---
    Gender: str
    Age: int
...
# ==========================================
# ENDPOINT UTAMA (PREDIKSI + GENERATE QUEST)
# ==========================================
...
async def generate_quests(user: UserInput):
    try:
        ...
        === DATA PENGGUNA ===
        - Nama: {user.Name}
        - BMI Faktual: {user_bmi} (Kategori: {bmi_category})
        - Target Turun Berat Badan: {user.Target_Weight_Loss} Kg

        === ANALISIS AI DEEP LEARNING (SANGAT PENTING) ===
        Meskipun BMI faktual user saat ini berada di kategori "{bmi_category}", model Deep Learning kami mendeteksi bahwa BERDASARKAN KEBIASAAN DAN GAYA HIDUPNYA SAAT INI, user memiliki profil risiko yang setara dengan: **{risk_class}**.
        
        ATURAN TINGKAT KESULITAN:
        - Jika "{risk_class}" adalah Normal_Weight / Insufficient_Weight: Berikan quest fisik menengah (push up, lari).
        - Jika "{risk_class}" adalah Overweight (Level I/II): Berikan quest fisik ringan (jalan kaki 5000 langkah, kurangi porsi ngemil).
        - Jika "{risk_class}" adalah Obesity (Type I/II/III): DILARANG keras menyuruh olahraga berat/melompat. Fokus pada nutrisi (minum 2 liter air, makan sayur), dan fisik sangat ringan (jalan santai 10 menit di tempat).
        ...
        # Memanggil API Gemini (Menggunakan await karena FastAPI bersifat asinkron)
